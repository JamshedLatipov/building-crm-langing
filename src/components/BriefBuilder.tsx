...
const STEPS = [
  {
    id: 1,
    title: "Project Type",
    questions: ["What are you building? (Web App, Mobile, AI Platform, etc.)"],
  },
  {
    id: 2,
    title: "Scope",
...
export const BriefBuilder: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (profile.industry === "Finance") {
      defaultAnswers["Are there specific security requirements?"] =
        "Strict PCI-DSS Compliance, End-to-End Encryption";
      defaultAnswers["Who are the target users?"] =
        "High Net Worth Individuals / Banking Clients";
    } else if (profile.industry === "Healthcare") {
      defaultAnswers["Are there specific security requirements?"] =
        "HIPAA Compliance, PII Data Anonymization";
      defaultAnswers[
        "What are you building? (Web App, Mobile, AI Platform, etc.)"
      ...
      defaultAnswers["Are there specific security requirements?"] =
        "FedRAMP Compliance, Sovereign Cloud";
    }
  ...
  const generateBrief = () => {
    ...
    setTimeout(() => {
      ...
      const goal =
        answers["What is the primary business goal?"] || "Not specified";
      const target = answers["Who are the target users?"] || "Not specified";
      const time = answers["What is your preferred timeline?"] || "TBD";
      const sec =
        answers["Are there specific security requirements?"] ||
        "Enterprise Standard";

      const pTypeLower = pType.toLowerCase();
      const techStack = pTypeLower.includes("mobile")
        ? "React Native / Flutter"
        : pTypeLower.includes("ai")
          ? "Python / LangChain / HuggingFace"
          : pTypeLower.includes("web")
      ...
      if (pTypeLower.includes("ai")) {
        ...
        aiRecs.push(
          "- Implement RAG with vector databases (Pinecone/Weaviate).",
        );
      } else if (pTypeLower.includes("mobile")) {
        aiRecs.push(
          "- Integrate on-device ML models for offline capabilities.",
        );
        ...
        <div className="bg-white rounded-3xl p-12 shadow-xl">
          <div className="mb-10 text-center">
            ...
            <p className="text-gray-500">
              Answer a few questions and let our AI draft your project roadmap.
            </p>
          </div>
          ...
          {!brief ? (
            <div className="space-y-8">
              <div className="flex justify-between mb-8">
                {STEPS.map((s) => (
                  <div
                    key={s.id}
                    className={`h-2 flex-1 mx-1 rounded-full ${step >= s.id ? "bg-[#0070ad]" : "bg-gray-200"}`}
                  ></div>
                ))}
              </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="flex space-x-4">
                <button className="flex-1 bg-[#0070ad] text-white py-4 rounded-xl font-bold">
                  Download PDF
                </button>
                <button
                  onClick={() => {
